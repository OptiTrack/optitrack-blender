...
class NatNetClient:
    ...
    print_level = 20 # changed from 20
    ...
    def connected(self): # associated with COMMAND thread
        ret_value = True
        # check sockets
    ...
    def __create_data_socket( self, port ):
        ...
        if self.use_multicast:
            ...
            result = socket.socket( socket.AF_INET,     # Internet
                                  socket.SOCK_DGRAM,
                                  0)    # UDP
            result.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) # allows the socket to reuse a local address and port 
            # RT: immediately after it's closed, even if there are active connections using the same address and port. 1 -> TRUE
            ...
            # RT: IPPROTO_IP -> option specific to the IP protocol, IP_ADD_MEMBERSHIP -> join multicast group on the specified interface
            # last part -> converts IP addresses from string format to binary representation (expected by socket functions) and concatenate
            # inet_aton() -> converts IP add from dotted-quad string format to 32-bit packed binary representation
            # RT: This is useful when the socket needs to receive multicast packets sent to a specific multicast group address.
            try:
            ...
            result = socket.socket( socket.AF_INET,     # Internet
                                  socket.SOCK_DGRAM,
                                  socket.IPPROTO_UDP) # RT: this is a default option for the protocol (but still explicitly stated)
            result.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            #result.bind( (self.local_ip_address, port) )
            try:
                result.bind( ('', 0) )
...
#Unpack Mocap Data Functions
    ...
    def __data_thread_function( self, in_socket, stop, gprint_level):
        ...
        while not stop():
            ...
            if len( data ) > 0 :
                #peek ahead at message_id
                message_id = get_message_id(data)
                tmp_str="mi_%1.1d"%message_id # RT: eg - mi_5
                if tmp_str not in message_id_dict:
                    message_id_dict[tmp_str]=0
                message_id_dict[tmp_str] += 1
                ...
                if message_id == self.NAT_FRAMEOFDATA:
                    if print_level > 0:
                        if (message_id_dict[tmp_str] % print_level) == 0:
                            ...
                            print_level = 0
                message_id = self.__process_message( data , print_level) # RT: printing the MoCap frame descriptions on the DATA thread
    ...
    def __process_message( self, data : bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA :
            ...
            offset += offset_tmp
            print("MoCap Frame: %d\n"%(mocap_data.prefix_data.frame_number)) # RT: MoCap frame modified, commented
            # get a string version of the data for output
            mocap_data_str=mocap_data.get_as_string()
            # pos_ori_data_loc, pos_ori_data_rot = mocap_data.retrieve_pos_ori() # RT: potentially want to save the location and orientation of the rigid body for every frame and output it
            if print_level >= 1: # RT: this is where the entire frame info is printed
                # print("%s\n"%mocap_data_str)
                pos_ori_data_loc, pos_ori_data_rot = mocap_data.retrieve_pos_ori()                
                # print("locs: %s,\n oris: %s\n"%(pos_ori_data_loc, pos_ori_data_rot)) # RT: potentially want to save the location and orientation of the rigid body for every frame and output it
        ...
        trace( "End Packet\n-----------------" )
        return message_id, pos_ori_data_loc, pos_ori_data_rot # RT: sending these variables
    ...
    def send_command( self, command_str):
        ...
        while nTries:
            ...
            if (ret_val != -1):
                break
        return ret_val # RT: D: in what case will the ret_val not return -1?
