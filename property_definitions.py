...
def update_skeleton_list(self, context):
    ...
    if m_id != "None":
        ...
        ske_rb_map = create_ble_ske_mapping(m_id, b_obj_id, existing_conn.assets_motive, current_obj)
...
class CustomObjectProperties(PropertyGroup):
    rigid_bodies : EnumProperty(name="Rigid Body", 
                                ...
                                update=update_rb_list, options={'SKIP_SAVE'})
    
    skeletons : EnumProperty(name="Skeleton", 
                                description="Assign objects in scene to skeleton IDs",
                                items=get_skeleton_id_names,
                                update=update_skeleton_list, options={'SKIP_SAVE'})
